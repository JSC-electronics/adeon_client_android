 ...
 */
package cz.jscelectronics.adeon.billingrepo
...
import cz.jscelectronics.adeon.billingrepo.BillingRepository.AdeonSku.CONSUMABLE_SKUS
import cz.jscelectronics.adeon.billingrepo.BillingRepository.AdeonSku.INAPP_SKUS
import cz.jscelectronics.adeon.billingrepo.BillingRepository.AdeonSku.SUBS_SKUS
import cz.jscelectronics.adeon.billingrepo.localdb.AugmentedSkuDetails
import cz.jscelectronics.adeon.billingrepo.localdb.Entitlement
import cz.jscelectronics.adeon.billingrepo.localdb.LocalBillingDb
...
class BillingRepository private constructor(private val application: Application) :
    ...
    /**
     * Tracks whether this user is entitled to an ads-free experience. This call returns data from the app's
     ...
     */
    val noAdvertisementsLiveData: LiveData<NoAdvertisements> by lazy {
        if (!::localCacheBillingClient.isInitialized) {
            localCacheBillingClient = LocalBillingDb.getInstance(application)
        }
        localCacheBillingClient.entitlementsDao().getNoAdvertisements()
    }
    ...
    private fun disburseNonConsumableEntitlement(purchase: Purchase) =
            CoroutineScope(Job() + Dispatchers.IO).launch {
                when (purchase.sku) {
                    AdeonSku.NO_ADVERTISEMENTS -> {
                        val noAdvertisements = NoAdvertisements(true)
                        insert(noAdvertisements)
                        localCacheBillingClient.skuDetailsDao()
                                .insertOrUpdate(purchase.sku, noAdvertisements.mayPurchase())
                    }
                }
